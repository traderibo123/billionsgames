/* start/game/end + logo fixes; demo/diagnostics girişte yok */
const GAME = { timeLimit:60, maxCards:100, deckEl:null, score:0, correct:0, total:0, time:60, timerId:null, name:'Player' };
...
document.addEventListener('DOMContentLoaded',()=>{
  ...
  // demo/diagnostics butonları kaldırıldığı için guard koyuyoruz (yoksa hiçbir şey yapmayız)
  const btnReal = $('btnReal'); if (btnReal) btnReal.addEventListener('click',()=>swipeTop('right'));
  const btnFake = $('btnFake'); if (btnFake) btnFake.addEventListener('click',()=>swipeTop('left'));
  const playAgain = $('playAgain'); if (playAgain) playAgain.addEventListener('click',()=>setScreen('screen-start'));
...
async function startGame(){
  ...
  let pool=[];try{pool=await buildPoolFromOnline();}catch(e){showError('Could not load handles.json or images.');setScreen('screen-start');return;}
...
async function buildPoolFromOnline(){
  ...
  if(!Array.isArray(all)||!all.length)throw new Error('No handles');
  const sh=all.slice().sort(()=>Math.random()-0.5);
  const batch=sh.slice(0, Math.max(20, Math.min(GAME.maxCards, all.length)));
  let id=1;
  const humans=batch.map(h=>{const src=unavatarFast(h);return{id:'card-'+(id++),src,realSrc:src,label:'human'};});
...
function createCard(it){
  ...
  card.append(img);GAME.deckEl.appendChild(card);attachGesture(card);
}
...
function attachGesture(card){
  ...
  const end=()=>{if(!drag)return;drag=false;const th=100;if(dx>th)swipe(card,'right');else if(dx<-th)swipe(card,'left');else{card.style.transition='transform .18s ease';card.style.transform='';}};
...
function swipe(card,dir){
  ...
  GAME.total++; if(ok){GAME.score++;GAME.correct++;} else {GAME.score=Math.max(0,GAME.score-1);}
